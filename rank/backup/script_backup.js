...
class BlogRankChecker {
  constructor() {
    ...
    this.extensionAvailable = false; // 확장 프로그램 사용 가능 여부
      ...
      { 
        ...
        speed: 'fast', 
        reliability: 'high',
        description: 'Freeboard 제공 (100K 문자 제한, 10req/s)'
      },
      { 
        name: 'AllOrigins', 
  ...
  toggleServer(serverIndex, isSelected, skipExclusiveLogic = false) {
    ...
    // 검색 중 프록시 변경 경고
    if (this.isSearching) {
      if (!confirm('검색 중입니다. 프록시를 변경하면 검색 결과가 불완전할 수 있습니다.\n계속하시겠습니까?')) {
        // 체크박스 원상복구
    ...
    if (isSelected) {
      // 배타적 선택 로직 완전 제거 - 모든 서버 동시 사용 가능
  ...
  autoSelectServers(forceUpdate = false) {
    ...
    if (onlineServers.length > 0) {
      ...
      console.log(`🌐 프록시 서버 ${onlineServers.length}개 자동 선택됨`);
    } else {
  ...
  findBlogLinks(html) {
    ...
    if (excludeRelatedPosts) {
      // 1. 기존 셀렉터로 제거
      const relatedElements = doc.querySelectorAll(`
        .link_box, .related_box, .series_box, .link_item, .related_link, .series_link,
        [class*="link_box"], [class*="related"], [class*="series"],
        .more_link, .additional_link, .tail_link, .append_link, .sub_link, .extra_link,
      ...
      // 2. 구분선 이후의 연관 링크 제거 (새로운 네이버 형식)
      const dividers = doc.querySelectorAll('[data-sds-comp="VertDivider"], .sds-comps-divider-vert');
      dividers.forEach(divider => {
        // 구분선의 부모 요소 찾기
        const parent = divider.closest('.RmWcK9JQ0jWyG868BWuw, [class*="RmWcK"]');
        if (parent) {
          console.log(`🚫 구분선 이후 연관 포스팅 제거`);
          parent.remove();
        }
      });
      
      // 3. 각 검색 결과 아이템 내에서 마지막 링크 제거
      const searchItems = doc.querySelectorAll('[data-template-id="ugcItem"], .FOnM43kEIDU_yHuzd71Q');
      searchItems.forEach(item => {
        // 아이템 내의 모든 링크 찾기
        const links = item.querySelectorAll('a[href*="blog.naver.com"]');
        if (links.length > 1) {
          // 마지막 링크가 연관 포스팅일 가능성이 높음
          const lastLink = links[links.length - 1];
          // data-heatmap-target이 .series인 경우 또는 마지막 링크인 경우
          if (lastLink.getAttribute('data-heatmap-target') === '.series' || 
              lastLink.closest('[data-heatmap-target=".series"]')) {
            const linkContainer = lastLink.closest('.DoVaYU3uUpQHU00uYhrW, .RmWcK9JQ0jWyG868BWuw');
            if (linkContainer) {
              console.log(`🚫 꼬리글 링크 제거: ${lastLink.textContent.substring(0, 30)}...`);
  ...
  async fetchNaverSearchResults(keyword, retryAttempt = 0) {
    ...
    console.log(`🔍 검색 시작: "${keyword}" | 선택된 프록시: ${proxyServers.length}개`, proxyServers);
    ...
    if (retryAttempt > 0) {
      ...
      console.log(`🔄 재시도 ${retryAttempt}: 프록시 순서 재정렬`);
    }
    ...
    for (let i = 0; i < proxyServers.length; i++) {
      ...
      if (proxy.includes('corsproxy.io')) {
        ...
        proxyName = 'ThingProxy';
      } else if (proxy.includes('allorigins')) {
        proxyName = 'AllOrigins';
      } else if (proxy.includes('cors.workers.dev')) {
        proxyName = 'Cloudflare';
      } else if (proxy.includes('codetabs')) {
        proxyName = 'CodeTabs';
      } else if (proxy.includes('cors-buster')) {
        proxyName = 'CORS-Buster';
      } else {
        // 기본 추출 방식
        proxyName = proxy.split('/')[2]?.split('.')[0] || 'Proxy';
      }
      ...
      try {
        ...
        // 타임아웃 단축 (5초 -> 3초부터 시작, 최대 5초)
        const timeout = Math.min(3000 + (i * 500), 5000);
        ...
        const statusTimer = setTimeout(() => {
          if (!controller.signal.aborted) {
            this.updateProxyStatus(`프록시 연결 중: ${proxyName}...`);
          }
        }, 1000);
        
        const fetchUrl = proxy + encodeURIComponent(naverBlogUrl);
        console.log(`🔗 프록시 URL [${i+1}/${proxyServers.length}]: ${fetchUrl.substring(0, 100)}...`);
        ...
        if (response.ok) {
          ...
          // 데이터 수신 - 타임아웃 제거하고 바로 처리
          const html = await response.text();
            console.log(`✅ 프록시 성공 [${i+1}/${proxyServers.length}] ${proxyName}: ${html.length}자 수신`);
        ...
        console.log(`❌ 프록시 실패 [${i+1}/${proxyServers.length}] ${proxyName}: ${errorMsg}`);
        ...
        // 잠시 대기 후 다음 프록시 시도 (800ms -> 300ms로 단축)
  ...
  async processBatch(keywords, batchSize = null) {
    // batchSize가 없으면 선택된 서버 수 * 2 사용 (병렬 처리 증가)
    if (!batchSize) {
      batchSize = Math.max(2, this.selectedServers.size * 2);
    }
        ...
        while (retryCount < MAX_RETRIES) {
          try {
            ...
            if (retryCount >= MAX_RETRIES) {
              ...
              // 재시도 대기 (2초, 4초 -> 1초, 2초로 단축)
              const waitTime = retryCount * 1000; // 1초, 2초 대기
  ...
  async checkRanks() {
    try {
      ...
      // 실패한 키워드가 있으면 자동으로 재시도
      if (failedKeywords.length > 0) {
        console.log(`⚠️ ${failedKeywords.length}개 키워드 검색 실패. 자동 재시도 시작...`);
        
        // 2초 후 실패한 키워드만 다시 검색
        setTimeout(async () => {
          console.log(`🔄 실패한 키워드 재검색: ${failedKeywords.join(', ')}`);
          
          // 실패한 키워드만 다시 검색
          const retryResults = await this.processBatch(failedKeywords, batchSize);
          
          // 재시도 결과로 기존 결과 업데이트
          retryResults.forEach(({ keyword, blogLinks, error }) => {
            if (!error && blogLinks.length > 0) {
              // 성공한 경우 기존 실패 결과를 업데이트
              allResults = allResults.map(result => {
                if (result.keyword === keyword) {
                  const rank = this.findRank(result.blogUrl, blogLinks);
                  return {
                    ...result,
                    rank,
                    totalBlogsInKeyword: blogLinks.length
              ...
              console.log(`✅ "${keyword}" 재검색 성공 (${blogLinks.length}개 블로그)`);
            }
          });
          ...
          const updatedStats = {
            ...stats,
            foundCount: allResults.filter(r => r.rank.found).length,
            failedKeywords: allResults.filter(r => r.rank.error).map(r => r.keyword)
          };
          
          // 화면 업데이트
          this.displayTableResults(allResults, updatedStats);
        }, 2000);
      }
