...
switch($method) {
    ...
    case 'POST':
        ...
        try {
            ...
            // JSON 인코딩 (안전한 방식)
            $categories = isset($input['categories']) ? json_encode($input['categories'], JSON_UNESCAPED_UNICODE) : '[]';
            $categoryPreviewCounts = isset($input['category_preview_counts']) ? json_encode($input['category_preview_counts'], JSON_UNESCAPED_UNICODE) : '{}';
            $selectedImages = isset($input['selected_images']) ? json_encode($input['selected_images'], JSON_UNESCAPED_UNICODE) : '[]';
